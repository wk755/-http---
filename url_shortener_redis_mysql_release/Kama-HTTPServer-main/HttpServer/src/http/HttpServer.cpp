...
void HttpServer::onRequest(const muduo::net::TcpConnectionPtr &conn, const HttpRequest &req)
{
    ...
    // —— 新增 begin：按请求版本补齐响应首行 ——
    std::string ver = req.getVersion();
    if (ver != "HTTP/1.0" && ver != "HTTP/1.1") ver = "HTTP/1.1";

    if (response.getHeader("Content-Length").empty() &&
        response.getHeader("Transfer-Encoding").empty()) {
        // 需要有 body() 访问器；若没有，见下方“没有 body() 的情况”
        response.addHeader("Content-Length", std::to_string(response.body().size()));
    }

    // HTTP/1.0 若未显式 Keep-Alive，则关闭连接并声明
    if (req.getVersion() == "HTTP/1.0" && req.getHeader("Connection") != "Keep-Alive") {
    ...
    int code = static_cast<int>(response.getStatusCode());
    ...
    response.setStatusLine(ver, response.getStatusCode(), reason);
    // —— 新增 end ——
